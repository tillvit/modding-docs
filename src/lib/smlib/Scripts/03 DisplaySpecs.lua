...
function ConfDisplayMode()
	...
	local choices = {}
	-- to avoid duplicates, add an index to the names.
	local indx_disp = 1
	for i, d in ipairs(specs) do
		choices[i] = d:GetName()
		if choices[i] == "" then
			choices[i] = "Display ".. indx_disp
			indx_disp = indx_disp + 1
		end
	end
	local origDisplay = PREFSMAN:GetPreference("DisplayId")
	local origWindowed = PREFSMAN:GetPreference("Windowed")
	choices[#choices + 1] = THEME:GetString("StepMania","Windowed")
	...
	local t = {
		...
		GenChoices= function(self)
			...
			foreach_ordered(ratios, function(v, f)
						vals[#vals + 1] = f.n/f.d
						choices[#choices + 1] = type(f.name) == "string" and f.name or string.format("%d:%d", f.n,f.d)
			end)
...
function ConfFullscreenType()
	...
	if fsbwSupported then
		choices = { THEME:GetString("StepMania","Exclusive"), THEME:GetString("StepMania","Borderless Window") }
