...
import { Container3D, Quaternion, Vec3 } from '../pixi3d.js';
	...
	init() {
		for (const [key, val] of Object.entries(this.options)) {
			if (key.endsWith('Command')) {
				this.commands.set(key.substring(0, -7), val);
...
export function createActor(options: Record<string, any>) {
	// console.log(read(fengari.L, 1));
	const actor = new Actor(options);
	// const metatable = actor.getMetatable()
